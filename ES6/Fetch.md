...
async function getProvince() {
    const url = 'https://free-api.heweather.net/s6/weather/now?location=常州&key=bd08d918fa9644a8b1359d0f704cc883';
    const config = {
        method: 'GET',
        haeder: {
            'Content-type': 'application/json', //请求的是json格式
            a: 1
        },
        // body: '{"a": "1" }' // 必须是post方法
    }
    const pro = await fetch(url, config);
